...
use crate::reactor::{
    Indicators, Io, IoEncode, IoImpl, IoTimeouts, IoTimeoutsRelative, Logger, ProtocolFlow, Reactor,
};
...
const COBS_BUFFER_LAYOUT: Layout = unsafe { Layout::from_size_align_unchecked(0xff, 1) };
const POSTCARD_ENCODE_BUFFER_LAYOUT: Layout =
    unsafe { Layout::from_size_align_unchecked(0x1000, 4) };
...
pub enum DriverError {
    ...
    #[error(
        "COBS frame ({total_encoded_length}) is declared longer than input buffer ({buffer_len})"
    )]
    InvalidFrameSize {
        total_encoded_length: usize,
        buffer_len: usize,
...
impl IrDriver {
    pub fn new(
        ...
        let frame_sink = FrameSink::new(cb, be, obuf_postcard);
    ...
    fn handle_error<L: Logger + ?Sized>(
        ...
        logger.writeln_fmt(
            reactor,
            format_args!("io_theseus: encountered error: {error}"),
        );
    ...
    fn state_tick<L: Logger + ?Sized, I: Indicators + ?Sized>(
        ...
        let new_state = match (maybe_byte, self.state) {
            ...
            | (Some(0x55), DriverState::AwaitingNextPacket) => DriverState::Preamble1,
            (Some(0x55), DriverState::Preamble1) => DriverState::Preamble2,
            (Some(0x55), DriverState::Preamble2) | (Some(0x55), DriverState::Preamble3) => {
                DriverState::Preamble3
            }
            (Some(0x5e), DriverState::Preamble3) => DriverState::FrameSize0,
            ...
            (_, state) => {
                if matches!(state, DriverState::AwaitingProtocolBegin) {
                    ...
                    let hit_session_timeout = self
                        .timeouts
                        .session_timeout
                        .as_ref()
                        .map(|st| packet_elapsed >= if st.use_long { st.long } else { st.long })
                        .unwrap_or(false);
                    ...
                    if hit_session_timeout && hit_byte_timeout {
                        ...
                        let _ = logger.writeln_fmt(
                            reactor,
                            format_args!("[device]: session expired after {packet_elapsed:?}"),
                        );
        ...
        let split_point: usize = match try {
            ...
            let split_point = match packet.len().checked_sub(4) {
                Some(i) => i,
                None => Err(DriverError::InsufficientLength(packet.len()))?,
            };
            let (data_frame_bytes, crc_bytes) = unsafe { packet.split_at_unchecked(split_point) };
...
impl Io for IrDriver {
    ...
    fn io_queue_message(
        ...
        let _ = logger.writeln_fmt(
            reactor,
            format_args!("queued message: {:?}", message.proxied_debug()),
        );
        match self.frame_sink.send_dyn(message) {
            Ok(x) => {
                ...
                for b in b1.into_iter().copied().chain(b2.into_iter().copied()) {
                    ...
                    logger.write_fmt(
                        reactor,
                        format_args!("{} ", core::str::from_utf8(&s).unwrap()),
                    );
                }
    ...
    fn io_tick(
        ...
        match self.ir_rx.tick(&reactor.peri.GPIO, &reactor.peri.SYSTMR) {
            Ok(b) => {
                // if let Some(b) = b {
            ...
            Err(ire) => {
                ...
                self.handle_error(reactor, logger, DriverError::Receive(ire))?
            }
        }
    }
