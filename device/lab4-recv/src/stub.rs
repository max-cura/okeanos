...
pub fn zero_stub_bss() {
    // SAFETY:
    //  noalloc so we're not writing over the heap; stack is-actually I'm not precisely sure where
    //  it is-but in any case we're assured by the linker script that nothing is in the way of the
    //  .bss section; it's just that we need to zero it because it's uninitialized and may be filled
    //  with garbage.
    // XXX: Stack is at the highest location
    unsafe {
        let start = core::ptr::addr_of_mut!(__symbol_bss_start__);
