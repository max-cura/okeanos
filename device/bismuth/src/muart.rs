...
use bcm2835_lpa::{Peripherals, AUX, GPIO, UART1};
use core::fmt::Write;

const MINI_UART_CLOCK_RATE: u32 = 250_000_000;

/// Calculate a value for the Mini UART clock divider from the desired baud rate.
pub const fn uart_baud_to_clock_divider(baud_rate: u32) -> u16 {
    ((MINI_UART_CLOCK_RATE / (8 * baud_rate)) - 1) as u16
}

pub fn __uart1_init(gpio: &GPIO, aux: &AUX, uart: &UART1, clock_divider: u16) {
...
pub fn __uart1_flush_tx(uart: &UART1) {
    __dsb();
...
pub fn __uart1_write_bytes_blocking(uart: &UART1, bytes: &[u8]) {
    __dsb();
...
pub struct UartBlockingFmt;
impl Write for UartBlockingFmt {
    fn write_str(&mut self, s: &str) -> core::fmt::Result {
        let peri = unsafe { Peripherals::steal() };
        __uart1_write_bytes_blocking(&peri.UART1, s.as_bytes());
...
#[macro_export]
macro_rules! uart1_sendln_bl {
    ($($args:tt)*) => {
        {
            let _ = <$crate::muart::UartBlockingFmt as ::core::fmt::Write>::write_fmt(&mut $crate::muart::UartBlockingFmt, format_args_nl!($($args)*));
        }
    }
}
