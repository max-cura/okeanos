...
impl Instant {
    ...
    pub fn elapsed_to(&self, current_time: Instant) -> Duration {
        Duration::from_micros(
            current_time
                .floating_micros
                .wrapping_sub(self.floating_micros),
        )
    }
}
...
pub fn delay_micros(st: &SYSTMR, microseconds: u64) {
    ...
    if core::intrinsics::unlikely(end < start) {
        if core::intrinsics::unlikely(microseconds > (u32::MAX as u64)) {
...
#[inline(always)]
pub fn cycle_read() -> u32 {
    let mut c: u32;
    unsafe { asm!("mrc p15, 0, {t0}, c15, c12, 1", t0 = out(reg) c) }
    c
}
